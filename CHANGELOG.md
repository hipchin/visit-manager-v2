...
- 訪問記録モーダルに、会えた時間帯とは別入力の「件数」欄を追加
- 設定画面に「表示設定」セクションを追加し、件数欄の表示をオン・オフできるトグルを追加
- 端末内設定を保存する`vm_settings`localStorageキーを追加（デフォルトはオフ、JSONバックアップの対象外）

### Changed

- 訪問履歴一覧で、件数欄がオンかつ値がある場合に件数を表示するよう変更
